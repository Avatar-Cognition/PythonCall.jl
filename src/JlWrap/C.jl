...
# lock protecting PYJLVALUES, PYJLFREEVALUES and PYJLFREEVALUECOUNT from
...
function _pyjl_dealloc(o::C.PyPtr)
    ...
    if idx != 0
        ...
        # just a write to an existing slot plus a counter bump. This avoids the
        # GC-triggered re-entrancy that caused the ConcurrencyViolationError:
        #   push!(PYJLFREEVALUES, …) → alloc → GC → py_finalizer →
        #     enqueue → Py_DecRef → _pyjl_dealloc → push! on same vector → BOOM.
        # The lock guards against true multi-thread races.
        lock(PYJL_LOCK)
...
PyJuliaValue_SetValue(_o, @nospecialize(v)) = Base.GC.@preserve _o begin
    ...
    if idx == 0
        ...
        if PYJLFREEVALUECOUNT[] == 0
            ...
            # is preserved. The 0 in PYJLFREEVALUES is just a placeholder; the slot
            # we just added is taken by `v`, not free. Growing only happens here, not
            # in the finaliser/deallocator path.
            push!(PYJLVALUES, v)
